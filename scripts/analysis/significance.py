...
The statistically significant HFBs are output to the specified directory as
`hfb_sgnf*.db` for analysis.
"""
...
def main(opt: Namespace):
    ...
    # Get a handle on Experiment.Trial
    logger.info(opt)
    ...
    # Get previously detected HFBs
    db = polydb.PNGDatabase.from_trial(trial, opt.chkpt, subdir=opt.subdir,
                                       delay_jitter=opt.delay_jitter, noise=opt.noise)
    if not db.exists:
        raise FileNotFoundError(f"Missing detections: '{db.path}'")
    logger.info(f"Loaded HFB detections: '{db.path}'")
    # Prepare HFB database
    db_sgnf_path = handler.get_hfb_path(
        trial, opt.chkpt, subdir=opt.subdir, sgnf=True,
        delay_jitter=opt.delay_jitter, noise=opt.noise
    )
    logger.info(f"Detections (significant) will be saved to: '{db_sgnf_path}'")
    if opt.no_action:
        return

    # === Significance testing === #

    if db_sgnf_path.exists():
        if opt.force:
            os.remove(db_sgnf_path)
            logger.info(f"Deleted existing file: '{db_sgnf_path}'")
        else:
            raise FileExistsError(f"Detections '{db_sgnf_path}' already exists: "
                                  "use '-f' flag to overwrite")
    db_sgnf = polydb.get_or_create_db(
        trial, opt.chkpt, subdir=opt.subdir, sgnf=True,
        delay_jitter=opt.delay_jitter, noise=opt.noise
    )
    ...
    spade_kwargs = {
        'n_surr': opt.n_surr,
        'winlen': opt.winlen // opt.bin_size,
        'bin_size': opt.bin_size
    }
    logger.info(f"SPADE custom parameters: {spade_kwargs}")
    pipeline.detect_significant_pngs(
        db_sgnf, db, rdb, layers=opt.layers, num_workers=opt.num_workers,
        spade_kwargs=spade_kwargs
    )
    t_elapsed = time() - t_start
    logger.info(f"Time for testing: {t_elapsed:.1f} s")


if __name__ == '__main__':
    parser = ArgumentParser(description="Significance test detected HFBs.")
    ...
    parser.add_argument('--n_surr', type=int, default=100)
